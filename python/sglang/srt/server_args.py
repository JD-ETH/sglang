...
class ServerArgs:
    ...
    inter_node_transfer_engine_info_port: Optional[int] = None  # Port for ZMQ inter-node transfer engine info communication
    ...
    nnodes: int = 1
    node_rank: int = 0
    node_hosts: Optional[str] = None  # Cross-node communication addresses: "host1:port1,host2:port2,..."
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            ...
            help="Comma-separated list of node addresses for cross-node communication (e.g., 'host1:port1,host2:port2'). "
            "If not provided, automatic discovery will be attempted.",
        )
    ...
    def init_new(
        ...
        if not server_args.enable_dp_attention:
            ...
            if server_args.inter_node_transfer_engine_info_port is not None:
                inter_node_transfer_engine_info_port = server_args.inter_node_transfer_engine_info_port
            else:
                inter_node_transfer_engine_info_port = server_args.port + INTER_NODE_TRANSFER_ENGINE_INFO_PORT_DELTA
            if dp_rank is None:
            ...
            try:
                if dp_rank is None:
                    ...
                    if server_args.nnodes > 1:
                        wait_port_available(inter_node_transfer_engine_info_port, "inter_node_transfer_engine_info_port")
            ...
            return PortArgs(
                ...
                tokenizer_worker_ipc_name=tokenizer_worker_ipc_name,
                inter_node_transfer_engine_info_port=inter_node_transfer_engine_info_port if server_args.nnodes > 1 else None,
            )
