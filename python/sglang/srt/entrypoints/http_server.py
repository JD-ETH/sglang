...
from sglang.srt.server_args import PortArgs, ServerArgs, INTER_NODE_TRANSFER_ENGINE_INFO_PORT_DELTA
...
from sglang.srt.model_loader.inter_node_transfer_engine_comm import cleanup_transfer_engine_info_comm
...
async def get_remote_instance_transfer_engine_info(rank: int = None):
    ...
    if (_global_state.node_registry and
        not _global_state.node_registry.validate_tp_rank(rank)):
        logger.error(f"Invalid tp_rank {rank}, must be 0 <= rank < {_global_state.node_registry.server_args.tp_size}")
        return Response(status_code=HTTPStatus.BAD_REQUEST)
    ...
    try:
        ...
        zmq_port = _get_target_node_zmq_port(target_node, _global_state.tokenizer_manager.server_args)
        logger.info(f"Using ZMQ to request rank {rank} from node {target_node.node_rank} "
