...
from sglang.srt.model_loader.inter_node_transfer_engine_comm import (
    ...
    get_transfer_engine_info_server,
)
...
def _launch_subprocesses(
    ...
    if server_args.node_rank >= 1:
        ...
        if server_args.nnodes > 1 and port_args.inter_node_transfer_engine_info_port is not None:
            # Parse transfer engine info from local scheduler_infos
            local_transfer_engine_info = parse_remote_instance_transfer_engine_info_from_scheduler_infos(scheduler_infos)

            if local_transfer_engine_info:
                # Initialize and start ZMQ server
                server = init_transfer_engine_info_server(
                    port_args.inter_node_transfer_engine_info_port,
                    server_args.node_rank
                )
                server.set_transfer_engine_info(local_transfer_engine_info)
                server.start()
                logger.info(f"Node {server_args.node_rank}: Started transfer engine info ZMQ server on port {port_args.inter_node_transfer_engine_info_port}")
