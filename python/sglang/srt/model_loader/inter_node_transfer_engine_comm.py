...
class TransferEngineInfoServer:
    ...
    def set_transfer_engine_info(self, transfer_engine_info: Dict[int, Tuple]):
        ...
        logger.info(f"Node {self.node_rank}: Set transfer engine info for ranks {list(transfer_engine_info.keys())}")

    def start(self):
        """Start the ZMQ server in a background thread."""
        if self.running:
            logger.warning(f"Node {self.node_rank}: Transfer engine info server already running")
            return

        self.running = True
        self.server_thread = threading.Thread(target=self._run_server, daemon=True)
        self.server_thread.start()
        logger.info(f"Node {self.node_rank}: Started transfer engine info server on port {self.port}")
    ...
    def _run_server(self):
        ...
        try:
            ...
            logger.info(f"Node {self.node_rank}: ZMQ server listening on port {self.port}")
...
class TransferEngineInfoClient:
    ...
    def get_transfer_engine_info(self, host: str, port: int, rank: int) -> Optional[Tuple]:
        """
        ...
        try:
            ...
            # Send request
            request = {
                "action": "get_transfer_engine_info",
                "rank": rank
            }
            ...
            if response.get("success"):
                ...
                logger.error(f"Request to {remote_address} failed: {response.get('error')}")
                return None
...
def get_transfer_engine_info_server() -> Optional[TransferEngineInfoServer]:
    """Get the global transfer engine info server instance."""
...
def init_transfer_engine_info_server(port: int, node_rank: int) -> TransferEngineInfoServer:
