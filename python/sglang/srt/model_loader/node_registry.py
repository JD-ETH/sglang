...
class NodeRegistry:
    ...
    def _populate_node_mapping(self):
        ...
        for node_rank in range(self.server_args.nnodes):
            ...
            logger.info(f"Node {node_rank}: host={host}, tp_ranks={list(node_info.tp_rank_range)}")

    def _get_node_host(self, node_rank: int) -> str:
        """
        Determine host for a given node_rank.
        """
        return getattr(self.server_args, 'host', '127.0.0.1') if self.noderank2host is None else self.noderank2host[str(node_rank)]
